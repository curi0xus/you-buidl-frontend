...
const BuidlItem = ({ contractAddress }) => {
  ...
  const projectStarter = projectDetails[0];
  const minContribution = projectDetails[1];
  const projectDeadline = projectDetails[2];
  const goalAmount = projectDetails[3];
  const completedTime = projectDetails[4];
  const currentAmount = projectDetails[5];
  const title = projectDetails[6];
  const desc = projectDetails[7];
  const currentState = projectDetails[8];
  const balance = projectDetails[9];
  const website = projectDetails[10];
  const social = projectDetails[11];
  const github = projectDetails[12];
  const projectCover = projectDetails[13];
